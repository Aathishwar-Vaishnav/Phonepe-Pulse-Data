...
mydb = mysql.connector.connect(host="localhost",user="root",password="sql54208")
mycursor = mydb.cursor(buffered=True,)
engine = create_engine("mysql+mysqlconnector://root:sql54208@localhost/phonepe") 
...
st.set_page_config(page_title='PHONEPE PULSE',page_icon=icon,initial_sidebar_state='expanded',
                        layout='wide',menu_items={"about":'This streamlit application was developed by Aathishwar Vaishnav S'})
...
if selected == "ABOUT":
        ...
        st.markdown('''<h5>Hello! I'm Aathishwar Vaishnav S, a BE graduate with a keen interest in data science and analytics.
                Currently on an exciting journey into the world of data science''',unsafe_allow_html=True)
        c1,c2=st.columns(2)     
        with c1:
                st.link_button('Linkedin','https://www.linkedin.com/in/aathishwar-vaishnav/') 
        with c2:
                st.link_button('Github','https://github.com/Aathishwar-Vaishnav')
...
if selected =="INSIGHTS":
        ...
        if select_insight =="TOP INSIGHTS":
                ...
                if query == 'Top 10 Brands used for Transaction':
                    col1, col2 = st.columns(2)
                    with col1:
                        # Execute the SQL query to get the top 10 brands with percentages
                        df = pd.read_sql_query('''
                            SELECT 
                                User_brand AS "User Brand", 
                                SUM(User_count) AS "Count", 
                                (SUM(User_count) / (SELECT SUM(User_count) FROM agg_user)) * 100 AS "Percentage"
                            FROM 
                                agg_user
                            GROUP BY 
                                User_brand
                ...
                if query=='Sum of Transaction by Type or categories':
                        col1,col2=st.columns(2)
                        with col1:
                                df=pd.read_sql_query('''SELECT DISTINCT Transaction_type as 'categories',
